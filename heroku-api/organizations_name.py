...
def recommend_companies(company_name, data, combine, transform):
        indices = pd.Series(data.index, index = data['name'])
        index = indices[company_name]
...
def results(company_name):
        ...
        if company_name not in find_company['name'].unique():
                return 'Company not in Database'

        else:
                recommendations = recommend_companies(company_name, find_company, combine_result, transform_result)
